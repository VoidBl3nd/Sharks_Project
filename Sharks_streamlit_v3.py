...
st.set_page_config(page_title="Sharky cruise builder", layout = "wide", initial_sidebar_state='collapsed') # must happen before any streamlit code /!\
st.markdown('<style>div.block-container{padding-top:3rem;}</style>', unsafe_allow_html=True) # remove blank top space
order_and_hide_pages(add_title=False)

st.subheader('Sharky Cruise Builder - :orange[*by FISHARK*]')
